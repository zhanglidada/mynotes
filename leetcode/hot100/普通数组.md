...
deorder[n - 1 - i] = deorder[n - i] * nums[n - i];

}

  

inorder[n - 1] = inorder[n - 2] * nums[n - 2];

deorder[0] = deorder[1] * nums[1];

  

for (int i = 0; i < n; i++)

res.push_back(inorder[i] * deorder[i]);
...
```
 ...
 2）因为输出数组不算在空间复杂度内，那么我们可以将 `L` 或 `R` 数组用输出数组来计算。先把输出数组当作 `L` 数组来计算，然后再动态构造 `R` 数组得到结果
