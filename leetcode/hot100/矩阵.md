...
分析将图像旋转 90 度之后，这些数字出现在什么位置，以下面的矩阵为例：
$$
\begin{matrix}
5&1&9&11 \\
2&4&8&10 \\
...
此时**第一行的第 x 个元素在旋转后恰好是倒数第一列的第 x 行元素**。
...
所以，对于矩阵中==第 i 行的第 j 个元素，在旋转后，它出现在倒数第 i 列的第 j 行位置==。
因为矩阵中的行列从 0 开始计数，因此对于矩阵中的元素 $matrix[row][col]$，在旋转后，它的新位置为 $matrix_{new}​[col][n−row−1]$。
...
因为旋转后$matrix[row][col]$的值存放到$matrix_{new}​[col][n−row−1]$的位置，所以需要一个临时变量tmp保存$matrix_{new}​[col][n−row−1]$的值。

代入新公式，原本$matrix_{new}​[col][n−row−1]$位置的数据会放入$matrix[n−row−1][n−col−1]$位置，同样地，直接赋值会覆盖掉 $matrix[n−row−1][n−col−1]$原来的值，因此我们还是需要使用一个临时变量进行存储，不过这次我们可以直接使用之前的临时变量 temp。

对于$matrix[n−row−1][n−col−1]$位置的值，转换后会覆盖$matrix[n−col−1][n - (n - row - 1) - 1] \rightarrow matrix[n−col−1][row]$ 位置的数据

对于$matrix[n−col−1][row]$位置的数据，经过旋转后会覆盖$matrix[row][n - (n - col - 1) - 1] \rightarrow matrix[row][col]$ 位置，即此次循环的开始位置。
...
![[Pasted image 20260113232119.png]]

此时可以保证不重复、不遗漏。

当 n 为奇数时，由于中心的位置经过旋转后位置不变，我们需要枚举 `(n^2 − 1)/4=((n−1)/2)*((n+1)/2)`个位置，需要换一种划分的方式，以 5×5 的矩阵为例：
![[Pasted image 20260113232403.png]]

此时同样可以保证不重复、不遗漏，同时矩阵正中央的点无需旋转。
...
示例 1：
![[Pasted image 20260114212058.png]]

**输入：** `matrix = [[1,4,7,11,15],[2,5,8,12,19],[3,6,9,16,22],[10,13,14,17,24],[18,21,23,26,30]], target = 5`
**输出：** true

示例 2：
![[Pasted image 20260114212121.png]]
...
解题思路：
1）最简单的思路就是顺序遍历整个矩阵，找到目标值。此时时间复杂度O(m*n)，空间复杂度O(1)

2）接着想到，因为每行都是顺序递增的，可以在每行使用二分查找的方式查找目标值。
时间复杂度：O(mlogn)，空间复杂度：O(1)。

3）又因为从上到下也是递增的，所以我们可以从矩阵的右上角开始遍历。从此位置开始，向下值增大，向左值减小，所以可以根据当前值和目标值的大小关系决定接下来的访问方向，直到到达边界位置。
间复杂度：O(m+n)，空间复杂度O(1)
