...
dm目前的实现应该是在存储过程，包以及方法中进行优化。
首先应该是统计常用的存储过程或者方法，如果发现某个方法的调用次数超过一定阈值，就开始尝试将对应的存储过程编译成机器码的方式去执行。

这里可以将一个存储过程编译成一个c方法，然后对其调用就是类似于对一个c方法，比如系统函数进行调用。这种方式应该是一种可行的调整方式。