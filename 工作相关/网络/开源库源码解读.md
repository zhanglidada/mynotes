...
libev 首先调用loop_init创建事件循环与后端。接着我们注册事件并添加 watcher，这里有几个注意事项：

用户通过 `ev_io_start`、`ev_timer_start`、`ev_signal_start` 等 API 添加监视器。
...
### 步骤 10：`EV_INVOKE_PENDING` —— 按优先级执行回调
...
- 若存在 cleanup watchers，先触发它们，给用户一次资源释放的机会。
...
初始化 → 注册 watcher（只做标记）
    ...
    ┌─ ev_run 循环 ────────────────────────────┐
    │  1. prepare 钩子                           │
    │  2. fd_reify 批量提交 I/O 监听              │
    │  3. 计算阻塞超时（查定时器堆顶）            │
    │  4. backend_poll 等待内核事件               │
    │  5. 就绪 fd 注入 pendings（egen 过滤）     │
    │  6. 时钟更新 + 时间跳跃修正                 │
    │  7. 到期定时器注入 pendings                 │
    │  8. 到期 periodic 注入 pendings             │
    │  9. idle / check 钩子                       │  
    │  10. INVOKE_PENDING 按优先级执行回调        │
    │  11. 判断 activecnt → 继续或退出            │
    └───────────────────────────────────────────┘
         ↓ 退出循环
    释放资源、关闭后端
...
一切从 `ev_default_loop` 或 `ev_loop_new` 开始，它们调用 `loop_init`：

1. **检测系统时钟**：验证单调时钟和实时时钟是否可用，初始化时间变量 (`mn_now`, `ev_rt_now`, `now_floor` 等)。
    
2. **搜索可用后端**：按优先级尝试初始化 I/O 多路复用后端，如 `epoll_init` → `kqueue_init` → `poll_init` → `select_init`。若启用 io_uring 且内核版本 ≥ 5.6.1，优先使用 io_uring。
    
3. **设置后端函数指针**：成功的后端把自己的 `modify` 和 `poll` 函数赋值给 `backend_modify` 和 `backend_poll`，`ev_run` 将完全依赖这两个指针工作。
    
4. **创建内部设施**：初始化用于信号/异步唤醒的 `evpipe`、用于时间跳变检测的 `timerfd`（可选）、以及 `signalfd`（可选）等。
    

此时，一个空的 `ev_loop` 已就绪，但还没有任何 watcher 被启动。

---

## 第二阶段：注册事件 —— 添加 watcher
...
## 第三阶段：主循环 —— `ev_run` 的每一轮迭代

这是 libev 的核心，每轮迭代严格按以下顺序执行：

### 步骤 1：处理 fork 与 prepare 钩子
...
### 步骤 5：`fd_event_nocheck` —— 将就绪事件注入 pending 队列
...
### 步骤 8：`periodics_reify` —— 处理到期周期任务

- 类似定时器，但基于实时时钟 `ev_rt_now`，支持绝对时间点和自定义重调度回调。
    

### 步骤 9：idle 与 check 钩子
...
libev具有统一的事件抽象，而裸 `epoll` 只能处理文件描述符的 I/O 就绪。libev 将**所有可能的异步事件源**抽象为统一的 `watcher`：

- `ev_io`：I/O 就绪
    
- `ev_timer`：一次性或重复定时器
    
- `ev_periodic`：基于绝对时间的周期任务
    
- `ev_signal`：信号处理（统一为事件，无需异步信号安全函数）
    
- `ev_child`：子进程状态变更（SIGCHLD 封装）
    
- `ev_stat`：文件系统变化
    
- `ev_idle`、`ev_prepare`、`ev_check`：循环生命周期钩子
...
libev 使用 **4 叉最小堆** 管理所有定时器：
- 获取最近超时时间：**O(1)**（直接读堆顶）。
- 插入/删除定时器：**O(log n)**。
- 支持海量定时器（数万甚至数十万），仍保持极低开销。
